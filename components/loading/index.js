  ...
  install (Vue) {
    const toggleLoading = (el, binding) => {
      ...
      if (binding.value) {
        ...
        Vue.nextTick(() => {
          ...
          if (binding.modifiers.locked) disableScroll()
          // If locked, disable scroll
        })
      } else {
        Vue.nextTick(() => {
    ...
    const removeLoading = (el, binding) => {
      if (el.isInsertDom) {
        ...
        delete el.mask
        if (binding.modifiers.locked) enableScroll()
        // Enable the scroll
      }
    }
    ...
    const disableScroll = (e) => {
      window.onwheel = window.ontouchstart = window.ontouchmove = window.onclick = prevent
      document.onwheel = document.ontouchstart = document.ontouchmove = document.onclick = prevent
    }
    // Disable scroll

    const enableScroll = (e) => {
      window.onwheel = window.ontouchstart = window.ontouchmove = window.onclick = null
      document.onwheel = document.ontouchstart = document.ontouchmove = document.onclick = null
    }
    // Enable scroll
